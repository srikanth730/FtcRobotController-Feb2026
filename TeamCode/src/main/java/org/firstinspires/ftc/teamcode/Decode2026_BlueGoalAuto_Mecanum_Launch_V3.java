...
/*
 * Decode 2026 Blue Goal Auto (relative moves) + launcher (from your TeleOp)
 *
 * Drive hardware names:
 *   left_front_drive, right_front_drive, left_back_drive, right_back_drive
 * Launcher hardware names:
 *   left_launcher, right_launcher, left_feeder, right_feeder, diverter
 * IMU name:
 *   imu
 */

@Autonomous(name = "A_V3_Decode 2026: Blue Goal Auto (0.8) + Launch", group = "Robot")
public class Decode2026_BlueGoalAuto_Mecanum_Launch_V3 extends LinearOpMode {
    ...
    // -------------------- TUNABLES --------------------
    static final double DRIVE_SPEED  = 0.80;   // per your request (was 0.40)
    static final double STRAFE_SPEED = 0.60;   // keep a bit lower than drive for accuracy
    ...
    // Limit gyro correction during RUN_TO_POSITION (helps stability at higher speed)
    static final double MAX_CORRECTION = 0.28;

    // -------------------- ROUTE PARAMETERS --------------------
    static final double BACKUP_TO_TRIANGLE_APEX_INCHES = 52.0;
    ...
    public void runOpMode() {
        ...
        while (opModeInInit()) {
            ...
            telemetry.addData("Back to apex (in)", "%.1f", BACKUP_TO_TRIANGLE_APEX_INCHES);
        ...
        // At START, current facing becomes 0° (goal-facing)
        imu.resetYaw();

        // ==================== AUTON SEQUENCE (your revised steps) ====================

        // 1) Start pose described (no code)

        // 2) Back 52 inches, no angle change
        driveStraight(DRIVE_SPEED, -52.0, HEADING_GOAL_DEG);

        // 3) Shoot 3 + wait 3 seconds
        shoot3ArtifactsAndWait3s(false);

        // 4) Turn left 45
        turnToHeading(TURN_SPEED, HEADING_LEFT45_DEG);

        // 6) Forward 24 (intake)
        driveStraight(DRIVE_SPEED, 24.0, HEADING_LEFT45_DEG);

        // 7) Back to step2 pose, facing goal
        driveStraight(DRIVE_SPEED, -24.0, HEADING_LEFT45_DEG);
        turnToHeading(TURN_SPEED, HEADING_GOAL_DEG);

        // 8) Shoot 3 + wait 3 seconds
        shoot3ArtifactsAndWait3s(false);
        ...
        // 12) Back to step2 pose, facing goal
        driveStraight(DRIVE_SPEED, -24.0, HEADING_LEFT45_DEG);
        strafeDistance(STRAFE_SPEED, 24.0, HEADING_LEFT45_DEG);
        turnToHeading(TURN_SPEED, HEADING_GOAL_DEG);

        // 13) Shoot 3 + wait 3 seconds
        shoot3ArtifactsAndWait3s(false);
        ...
        // 19) Back to step2 pose, facing goal
        driveStraight(DRIVE_SPEED, -24.0, HEADING_LEFT45_DEG);
        strafeDistance(STRAFE_SPEED, 36.0, HEADING_LEFT45_DEG);
        turnToHeading(TURN_SPEED, HEADING_GOAL_DEG);

        // 20) Shoot 3 + wait 3 seconds (WITH empty line placeholder)
        shoot3ArtifactsAndWait3s(true);
    ...
    private void shoot3ArtifactsAndWait3s(boolean includeEmptyLinePlaceholder) {

        if (includeEmptyLinePlaceholder) {
            // (Empty code line placeholder you requested for step 20)
        ...
        // Wait 3 seconds to finish shooting (per your instructions)
        sleep(3000);
    ...
    public void driveStraight(double maxDriveSpeed, double distanceInches, double headingDeg) {
        ...
        while (opModeIsActive() && anyBusy()) {
            ...
            // If driving backwards, reverse correction sign
